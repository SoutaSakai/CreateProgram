...
void Player::Update(void)
{
	...
	if (SkilFlag)
	{
		...
		case CHARACTER_ID::TUNA:
			tuna->Update(CharacterPos, Angle, Scale.x);
			break;
		default:
			break;
		}
...
void Player::Controller(void)
{
	...
	if (vivid::controller::Button(m_PlayerID, vivid::controller::BUTTON_ID::RIGHT_SHOULDER))
	{
		...
		switch (UseCharacter[playermanager::GetInstance().GetRoundCount()])
		{
		case CHARACTER_ID::DUMMY:
			break;
		case CHARACTER_ID::ELSCTRICEEL:
...
void Player::Draw()
{
	...
	//確認コード===>
	vivid::DrawText(40, std::to_string(CharacterPos.x),	vivid::Vector2(0.0f, 0.0f));
